    ...
    <div class="box-body fields-group">
                ...
                <select class="form-control parent_id select2-hidden-accessible" style="width: 100%;"
                    ...
                    @foreach($menu->menus->where('parent_id',0)->sortBy('sort') as $taxonomy)
                        ...
                        @if(count($taxonomy->parents))
                            @include('admin.menu.select-sub-category',['subcategories'=>$taxonomy->parents,'iteration'=>1])
                        @endif
                    @endforeach
                </select>
            </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <div class="input-group">
                    <span class="input-group-addon"><i class="fa fa-pencil fa-fw"></i></span>
                    <input type="text" id="uri" name="parameters"
                           @isset($menuItem)
                               value="{{ $menuItem->parameters }}"
                           @else
                               value="{{ old('parameters') }}"
                           @endisset
                           class="form-control uri" placeholder="Ввод параметров">
                </div>
            </div>
        </div>
    </div>
